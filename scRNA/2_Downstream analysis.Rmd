...
dat.combined <- readRDS('../../output/RNA/breast_cancer_scRNA_processed.Rds')
...
# Change assay
DefaultAssay(object = dat.filtered) <- "tfsulm"
dat.filtered <- Seurat::ScaleData(dat.filtered)
dat.filtered@assays$tfsulm@data <- dat.filtered@assays$tfsulm@scale.data


deg_TF <- data.frame()
for(i in unique(dat.filtered$cell_type_major)){
  sub <- dat.filtered[, dat.filtered$cell_type_major == i]
  tryCatch({
    degs <- FindMarkers(sub, assay = 'tfsulm', group.by = 'tissue', ident.1 = 'TUMOR', ident.2 = 'NORMAL')
    degs <- degs %>% rownames_to_column(var = 'gene') %>% mutate(cell = i)
    deg_TF <- rbind(deg_TF, degs)
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}

deg_TF_signif <- deg_TF %>% dplyr::filter(p_val_adj < 0.05 & abs(avg_log2FC) > log2(1.5))
deg_TF_signif_list <- split(deg_TF_signif, deg_TF_signif$cell)


for(i in 1:length(deg_TF_signif_list)){
  temp <- deg_TF_signif_list[[i]]
  temp <- temp %>% dplyr::arrange(p_val_adj) %>% head(.,20)
  
  plot1 <- ggplot(data = temp, mapping = aes(x = reorder(gene, avg_log2FC), y = avg_log2FC, fill = -log10(p_val_adj)))+
    geom_col()+
    cowplot::theme_cowplot() +
    labs(x = "", y = "log2FC average", title = names(deg_TF_signif_list[i])) +
...
Regulon_df <- read.csv("../../output/RNA/SCENIC_Output/MSC_regulon.csv")
Regulon_mtx <- read.csv("../../output/RNA/SCENIC_Output/MSC_auc_mtx.csv")
GRN <- read.csv("../../output/RNA/SCENIC_Output/MSC_adj.csv")
